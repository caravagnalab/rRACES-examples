...
t2=50
# first sample after therapy
t3=40
# second sample after therapy
t4=100
# third sample after therapy
t5=50
...
sample.t5.B <- sim$get_counts()$counts[2]
sum_stat_4 <- (sample.t5.B - sample.t4.B)/sample.t4.B
...
# Model definition ---------------------------------------------------
create_model <- function(x1, y2, y3, y4, y5){
    model=function(x){
        if (all(x > 0)){
            # Create simulation
            ...
            tmp$run_up_to_time(tmp$get_clock() + y3)
            ...
            sample.t5.B <- tmp$get_counts()$counts[2]
            sum_stat_4 <- (sample.t5.B - sample.t4.B)/sample.t4.B
...
# Create the gr_model
m <- create_model(gr.A, t2, t3, t4, t5)

nb_simul=1000
tr=0.20
prior = list(c("unif", 10, 150), c("unif", 0.03, 0.1))
