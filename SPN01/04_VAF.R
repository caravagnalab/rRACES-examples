...
library(ggplot2)
library(patchwork)
source("../SPN04/utils.R")

data <- readRDS("data/sequencing_homogeneous_growth.rds")
BASE_FACTOR <- 1e6
...
for (curr_chr in data$chr %>% unique()) {
  ...
  ggsave(paste0("tissue_homo/chromosomes/", curr_chr, ".png"), dpi = 400, width = 6, height = 8, plot = pp)
  print(curr_chr)
}
