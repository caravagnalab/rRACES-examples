...
chromosomes <- c("22","5")
seq_results <- parallel::mclapply(chromosomes, function(c) {
					          simulate_seq(phylo_forest, chromosomes = c, coverage = 50,write_SAM = TRUE,read_size =150,
							                            sequencer = basic_seq, insert_size_mean = 350, insert_size_stddev = 10,output_dir = "/orfeo/LTS/CDSLab/LT_storage/ggandolfi/races_simulations/CHECK_PURITY/sequencing_50X_basic_error_paired_350_spn01_new",
										            update_SAM =FALSE, with_normal_sample =FALSE)
}, mc.cores = 2)


seq_results_final<- do.call("bind_rows", seq_results)
saveRDS(seq_results_final, "/orfeo/cephfs/scratch/cdslab/ggandolfi/prj_races/rRACES-examples/SPN01/data/seq_results_WGD_chr5_22.rds")
