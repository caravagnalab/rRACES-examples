...
merged_freebayes = merge_datasets(snp_caller = list_tool_pass$freebayes, ground_truth = process_normal)
...
merged_strelka = merge_datasets(snp_caller = list_tool_pass$strelka, ground_truth = process_normal)
...
merged_haplotypecaller = merge_datasets(snp_caller = list_tool_pass$haplotypecaller, ground_truth = process_normal)
...
report_plot <- DP_density + theme(legend.position = 'none') + DP_ecdf + theme(legend.position = 'none') +
  BAF_density + theme(legend.position = 'none') + BAF_ecdf + theme(legend.position = 'none') +
  upset_plot + metric_plot +
  plot_layout(design = design, guides = 'collect') +
  plot_annotation(title)

ggsave(plot = report_plot, filename = paste0(report,'/normal.png'), dpi = 500, width = 10, height = 12, units = 'in')
saveRDS(object = list(report_metrics=all_metric), file = paste0(report,'/', tool, '_normal_metrics.rds'))