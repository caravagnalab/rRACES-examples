...
source("/orfeo/cephfs/scratch/cdslab/ggandolfi/prj_races/REPO_UPDATED/rRACES-examples/report/plotting/tables.R")
...
plot_stats_sample(params, sample_forest)
```
...
drivers_long <- s_seq %>% filter(classes=="driver") %>% 
  seq_to_long() %>% 
...
dir <- paste0('/orfeo/scratch/area/lvaleriani/data_SPN01', '/', params$spn, '/', coverage, 'x_', purity, 'p/rds/')
som <- readRDS(paste0(dir, 'seq_results_cna_muts_somatic.rds'))
upper <- 0.89
lower <- 0.11
...
clonal_plots <- lapply(split_list,function(list){
  whole_genome <- clonal %>%
    ...
    xlim(-0.01, 1) +
    ggh4x::facet_nested(sample_name ~ chr, scales = 'free_y', independent = 'y') +
...
fix_n_samples_per_height <- 3
palette_labels <- RColorBrewer::brewer.pal(n = length(unique(model_df$label)),name = "Set2")
