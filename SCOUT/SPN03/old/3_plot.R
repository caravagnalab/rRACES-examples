...
seq_results <- readRDS('/orfeo/LTS/LADE/LT_storage/lvaleriani/races/SPN03/results/LAST_seq_100X.RDS')
...
#  p <- p1 / p2 / p3
#  ggsave(paste0("plots/LAST_SPN03_",sample, '_report.png'),plot = p, dpi=300, width = 8, height = 10)
#}

pdf("plots/FULL_LAST_SPN03_vaf_report.pdf", width = 8, height = 10)
...
for (c in unique(s_seq$chr)) {
    ...
    p_hist <- plot_VAF_histogram(s_seq, chromosomes = c, samples = samples, labels = s_seq["classes"], cuts = c(0.001, 1)) + xlim(0,1)
