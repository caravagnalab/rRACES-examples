...
```{r}
m_engine$add_mutant(mutant_name = "Clone 1", passenger_rates = c(SNV = 1e-8, CNA = 1e-12, indel = 1e-9))
```

# Reference path
The references to generated the MutationEngine are stored in /orfeo/cephfs/scratch/cdslab/shared/ProCESS/GRCh38. 
Please when setting the MutationEngine do:
```{r}
...
The following output files must be saved in the directory: /orfeo/cephfs/scratch/cdslab/shared/SCOUT/SPNX/process (replace X with the number of your SPN):
