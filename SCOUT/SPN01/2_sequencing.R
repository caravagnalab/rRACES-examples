...
seq_results <- parallel::mclapply(chromosomes, function(c) {
					  	simulate_seq(phylo_forest, chromosomes = c, coverage = 200,write_SAM = FALSE,read_size =150, 
							       		     sequencer = basic_seq, purity=0.9,insert_size_mean = 350, insert_size_stddev = 10, 
									     update_SAM =FALSE, with_normal_sample =TRUE,output_dir=paste0(dir,"data"))
}, mc.cores = 8)

seq_results_final<- do.call("bind_rows", seq_results)
saveRDS(seq_results_final, paste0(dir,"/seq_res_smaller.rds"))
