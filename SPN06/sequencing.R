...
seq_results <- parallel::mclapply(
  ...
  function(c) {
    simulate_seq(
      phylo_forest = phylo_forest, sequencer = basic_seq, chromosomes = c, coverage = 50, write_SAM = FALSE, purity = 1, with_normal_sample = TRUE
    )
  }, 
...
saveRDS(seq_results_final, file = paste(curr_dir, "/data/SPN06_seq_50X.rds", sep = "") )
